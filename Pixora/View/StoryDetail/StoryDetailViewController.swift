...
//  Pixora
//
//  Created by İrem Sever on 3.09.2024.
//

import UIKit

class StoryDetailViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource {
    @IBOutlet weak var collectionViewStoryDetail: UICollectionView!
    private let viewModel = StoryViewModel()
       var selectedIndex: Int = 0
       var selectedStory: StoryResponse?
       ...
       func loadData() {
           ...
           viewModel.storyData = [selectedStory]
       ...
       func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
           ...
           if let story = viewModel.getStory(at: indexPath.row) {
       ...
       func didStoryViewEnd() {
           if selectedIndex < viewModel.storyData.first?.storyDetails.count ?? 0 - 1 {
               selectedIndex += 1
               performCubeTransition(toNext: true)
           } else {
               dismiss(animated: true, completion: nil)
